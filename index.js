...
//APIKEY
zeksApi = ["TwCfNZjrTXYBghQQsMUOuRYPVbU"]
...
//FAKE VIDEO
const flexx = {
key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6285824629954-1613049930@g.us" } : {}) },message: { "videoMessage": { "title":"hallo bang","h": `Hmm`,'seconds': '-99999', 'caption': `${fakeyoi}`,'jpegThumbnail': fs.readFileSync('./gambar/fake.jpg')}}}
//FAKE GRUP
const fgc = {
...
"listMessage": {
"title": '「 STORE MENU 」',
"description": `*Created By VANZZ OFC*`,
"buttonText": "𝙋𝙄𝙇𝙄𝙃 𝘿𝙄𝙎𝙄𝙉𝙄",
"listType": "SINGLE_SELECT",
"sections": [
{
...
Lexxy.sendMessage(from, bf, image, { quoted: flexx, caption: 'Logo By VANZZ OFC' })
break
case 'kaneki':
case 'rem':
case 'lolimaker':
...
Lexxy.sendMessage('6285824629954@s.whatsapp.net', options, text, {quoted: flexx})
Lexxy.sendMessage('6285824629954@s.whatsapp.net', options, text, {quoted: flexx})
...
for (let _ of anu) {
Lexxy.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「  」*\n\n${body.slice(4)}`})
}
...
publik = true
fakeText('*𝙎𝙐𝙆𝙎𝙀𝙎 𝙈𝙀𝙉𝙂𝙐𝘽𝘼𝙃 𝙆𝙀 𝙈𝙊𝘿𝙀 𝙋𝙐𝘽𝙇𝙄𝘾*')
break
case 'self':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
publik = false
fakeText('*𝙎𝙐𝙆𝙎𝙀𝙎 𝙈𝙀𝙉𝙂𝙐𝘽𝘼𝙃 𝙆𝙀 𝙈𝙊𝘿𝙀 𝙎𝙀𝙇𝙁*')
break
case 'sticker':
case 's':
if (isBanned)return sticBanned(from)
...
but = [
{ buttonId: `${prefix}ply4 ${args.join(" ")}`, buttonText: { displayText: '𝙑𝙄𝘿𝙀𝙊' }, type: 1 },
{ buttonId: `${prefix}ply3 ${args.join(" ")}`, buttonText: { displayText: '️𝙈𝙐𝙎𝙄𝙆' }, type: 1 }
]
